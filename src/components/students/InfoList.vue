...
<script>
import { InfoApi, delInfoApi } from "@/api/api.js";
export default {
  data() {
    return {
      tableData: [],
    ...
    onSure(newForm) {
      this.$refs[newForm].validate((valid) => {
        if (valid) {
          // 判断新增 / 修改
          if (this.state) {
            InfoApi(this.newForm, "post").then((res) => {
              if (res.data.status === 200) {
                this.tableData = res.data.data;
                this.tableData.forEach((el) => {
                  el.sex == "1" ? (el.sex_text = "男") : (el.sex_text = "女");
            ...
            InfoApi(this.newForm, "put").then((res) => {
              if (res.data.status === 200) {
                this.tableData = res.data.data;
                this.tableData.forEach((el) => {
                  el.sex == "1" ? (el.sex_text = "男") : (el.sex_text = "女");
                ...
                this.$message({ type: "danger", message: "修改数据失败" });
              }
            });
          }
        }
